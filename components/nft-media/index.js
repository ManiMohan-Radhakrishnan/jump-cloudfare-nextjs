        ...
        >
          ...
          {nftRoleData && (
            <div className="player-type">
              ...
              <img src={nftRoleData?.value} alt="Player-type" loading="lazy" />
            </div>
          )}
          {raddxCarCategory && (
            <div className="player-type">
          ...
          {batData && (
            <div className="bat-type-2x">
              <h6>{batData?.name}</h6>
              <img src={batData?.value} alt="Bat-Power" loading="lazy" />
            </div>
          )}
            ...
            {(() => {
              if (nft?.asset_type?.includes("image")) {
                return isLand ? (
                  <ReactPanZoom
                    alt={`${nft?.name}`}
                    image={nft?.asset_url ? nft?.asset_url : images.sample}
                    disableFlip
                    disableReset
                    disableRotate
                  />
...
const BundleAsset = ({ nft, bundle = {}, switchOrderBundle = () => {} }) => {
  const handleBundleClick = () => {
    console.log("asset", bundle);
    console.log("nft", nft);
    switchOrderBundle(bundle?.nft_slug ? bundle?.nft_slug : bundle?.slug);
  };
  return (
    <div
      className={`bundle ${
    ...
    >
      {bundle?.asset_type?.includes("image") ? (
        <img
          alt={`${bundle?.name}`}
          className="type_image typeimg_audio"
          src={bundle?.asset_url ? bundle?.asset_url : images.sample}
          loading="lazy"
        />
